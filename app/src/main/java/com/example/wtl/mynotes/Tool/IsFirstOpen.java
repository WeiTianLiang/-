...
public class IsFirstOpen {
    ...
    public static void IsFirstOpen(ImageView handle_list, SharedPreferences preferences, Boolean user_first, Context context, SQLiteDatabase readbase, List<Notes> notesList, RecyclerView handle_recycler, Animation animation) {
        if(user_first){//第一次
            preferences.edit().putBoolean("FIRST", false).apply();
            ReadCuesor.ReadCuesor(context,0,readbase,notesList,handle_recycler,animation);
            ...
            ReadCuesor.ReadCuesor(context,x,readbase,notesList,handle_recycler,animation);
        }
    }
